...
### 2.2. Classes `SimpleProcessor`, `Processor`, and `Flext`

**`SimpleProcessor` in `core/`:**
...
* `@group`, `@option` — metadata grouping and field value options
* `@lang`, `@title`, `@timeZone`, `@margins` — template-level metadata
...
* `media`, `string`, `array` — media and utility helpers
...
* ❌ Don’t change directive formats (`@syntax`, `@use`, `@lineHeight`, `@field`, `@group`, `@option`, etc.) without updating regexes, logic, docs, tests
* ❌ Don’t change the semantics of built-in modules (`put`, `math`, `cond`, `match`, `date`, `number`, `media`, `string`, `array`)
...
* [ ] `npm run build` and `npm run test` pass in every affected package
