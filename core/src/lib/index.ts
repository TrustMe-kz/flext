...
export class HandlebarsCommentCollector extends HandlebarsCollector<string> {
...
export class HandlebarsContentCollector extends HandlebarsCollector<string> {
...
export class HandlebarsPathCollector extends HandlebarsCollector<string> {
...
export class FlextMacroCollector extends HandlebarsCommentCollector {
    public match = FilterHelper.macro;
}

export class FlextH1SomewhereContentCollector extends HandlebarsContentCollector {
...
export function hbsToAst(val: types.Hbs): AST.Program {
    return Handlebars.parse(val);
}

export function hbsToDelegate(val: types.Hbs | AST.Program): TemplateDelegate {
    return Handlebars.compile(val);
}

export function delegateToHtml(template: TemplateDelegate, data: types.Obj = {}, helpers: types.Obj = {}): types.Html {
    return template(data, { helpers });
}

export async function delegateToCss(template: TemplateDelegate, data: types.Obj = {}, options: types.Obj = {}): Promise<types.Css> {
    ...
    const html = delegateToHtml(template, data, helpers);
...
export function astToPaths(ast: AST.Program): string[] {
...
export function astToDataModel(ast: AST.Program): types.DataModel {
    const [ first, ...paths ] = astToPaths(ast);
...
export function macroTokenToMacroParam(val: string): types.MacroParam | null {
...
export function macroTokenToMacroParams(val: string, doWarn: boolean = true): types.MacroParam[] {
    ...
    for (const token of matches) {
        const macro = macroTokenToMacroParam(token);
...
export function astToMacros(ast: AST.Program, doWarn: boolean = true): types.Macro[] {
    ...
    for (const macroStr of macroArr) {
        ...
        const params = macroTokenToMacroParams(paramsStr, doWarn);
...
export function delegateToHtmlH1(ast: AST.Program, doWarn: boolean = true): string[] {
...
export function hbsToDelegateValidationErrorsByMetadata(data: types.Obj, model: types.MetadataModelNode[], depth: number = DEFAULT_MODEL_DEPTH): TemplateDataValidationError[] {
    ...
    for (const node of model) {
        ...
        result.push(...hbsToDelegateValidationErrorsByMetadata(fieldValue ?? {}, node.$ as types.MetadataModelNode[], depth - 1));
    }
